...
It is possible that the default colors provided by `RCaucTile` are not suitable for your plots. The argument `tile_colors` will help to change the colors of numeric variables to one of the predefined palettes from the `RColorBrewer` package (`Blues`, `BuGn`, `BuPu`, `GnBu`, `Greens`, `Grey`, `Oranges`, `OrRd`, `PuBu`, `PuBuGn`, `PuRd`, `Purples`, `RdPus`, `Reds`, `YlGn`, `YlGnBu`, `YlOrBr`, `YlOrRd`) or the `viridis` package (`viridis`, `inferno`, `magma`, `plasma`). There is a `palette_reverse` argument that makes it possible to reverse the order of the colors in the palette.
...
To define your own palette for a numeric variable, you can provide 2 or 3 colors[^about_color_names] defining the lower, upper, and middle segments of the palette.

[^about_color_names]: R has over 650 built-in color names that you can use in your plots, which can be accessed by typing `colors()`. It is also possible to use the HEX value of the color, e.g. `"#0000FF"`.
...
ec_tile_map(ec_languages,
            ...
            title = "Consonant inventory size (Moroz 2021)",
            annotate_feature = TRUE,
            tile_colors = c("navy", "tomato"))

ec_tile_map(ec_languages,
            feature_column = "consonant_inventory_size",
            title = "Consonant inventory size (Moroz 2021)",
            annotate_feature = TRUE,
            tile_colors = c("navy", "tomato", "yellow"))
```

When the color scheme is clear and the `annotate_feature` argument draws the exact values of the feature, it makes sense to get rid of the legend using the `theme()` function from the `ggplot2` package:
...
The `tile_colors` argument will help to change the colors of categorical variables to one of the predefined palettes (`Accent`, `Dark2`, `Paired`, `Pastel1`, `Pastel2`, `Set1`, `Set2`, `Set3`).
...
To define your own palette for a categorical variable, you can use the `tile_colors` argument. Just keep an eye on prodviding the same amount of colors as you have in the feature variable.

```{r}
ec_tile_map(ec_languages,
            feature_column = "morning_greetings",
            title = "Morning greetings (Naccarato, Verhees 2021)",
            tile_colors = c("#D81E05", "#0070A1", "#00923F"))
```

## Changing values' order

The default order of values in R is alphabetical. To impose a different order, you can use the `factor()` function. This function doesn't change the values themselves, but it instructs R to treat an input vector as ordered. This can be important for features with a natural order.  For example, the following demonstrates how to override the default alphabetical order using `factor()`.

```{r}
df <- data.frame(language = c("Avar", "Chechen", "Mehweb"),
                 feature = c("value a", "value b", "value b"))

df$feature <- factor(df$feature, levels = c("value b", "value a"))
