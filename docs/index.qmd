...
It is possible that the default colors provided by `RCaucTile` are not suitable for your plots. The argument `tile_colors` will help to change the colors of numeric variables to one of the predefined palettes from the `RColorBrewer` package (`Blues`, `BuGn`, `BuPu`, `GnBu`, `Greens`, `Grey`, `Oranges`, `OrRd`, `PuBu`, `PuBuGn`, `PuRd`, `Purples`, `RdPus`, `Reds`, `YlGn`, `YlGnBu`, `YlOrBr`, `YlOrRd`) or `viridis` package (`viridis`, `inferno`, `magma`, `plasma`). There is a `palette_reverse` argument that makes it possible to reverse the order of the colors in the palette.
